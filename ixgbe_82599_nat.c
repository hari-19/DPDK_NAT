	...
	for(;;) {
		...
		for(int i=0;i<nb_rx;i++) {
			...
			if (eth_hdr->ether_type == rte_cpu_to_be_16(ARP)) {
				...
				struct rte_arp_hdr *arphdr = (struct rte_arp_hdr *)(rte_pktmbuf_mtod(single_pkt, unsigned char *) + sizeof(struct rte_ether_hdr));
			...
			struct rte_ipv4_hdr *ip_hdr = (struct rte_ipv4_hdr *)(rte_pktmbuf_mtod(single_pkt, unsigned char *) + sizeof(struct rte_ether_hdr));
	...
	for(;;) {
		...
		for(i=0; i<nb_rx; i++) {
			...
			switch(ip_hdr->next_proto_id) {
				case UDP :
					single_pkt->ol_flags |= RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_UDP_CKSUM;
					udphdr = (struct rte_udp_hdr *)(rte_pktmbuf_mtod(single_pkt, unsigned char *) + sizeof(struct rte_arp_hdr) + sizeof(struct rte_ipv4_hdr));
	...
	for(;;) {
		...
		for(i=0; i<nb_rx; i++) {
			...
			if (eth_hdr->ether_type == rte_cpu_to_be_16(ARP)) {
				...
				rte_memcpy(eth_hdr->src_addr.addr_bytes,mac_addr[0],6);
				struct rte_arp_hdr *arphdr = (struct rte_arp_hdr *)(rte_pktmbuf_mtod(single_pkt, unsigned char *) + sizeof(struct rte_ether_hdr));
				if (arphdr->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REQUEST) && arphdr->arp_data.arp_tip == ip_addr[0]) {
					rte_memcpy(arphdr->arp_data.arp_tha.addr_bytes,arphdr->arp_data.arp_sha.addr_bytes,6);
